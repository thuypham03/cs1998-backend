...
from db import Submission, db, Course, User, Assignment
...
def create_submission(assignment_id):
    ...
    body = json.loads(request.data)
    user_id = body.get("user_id", -1)
    content = body.get("content", -1)
    if user_id == -1 or content == -1:
        return failure_response("Missing request information")
    ...
    if not user.hasCourse(assignment.course_id):
        return failure_response("User does not have this assignment")
...
def grade_submission(assignment_id):
    """
    Endpoint for grading a submission
    """
    body = json.loads(request.data)
    submission_id = body.get("submission_id", -1)
    score = body.get("score", -1)
    if submission_id == -1 or score == -1:
        return failure_response("Missing request information")
    ...
    if submission.assignment_id != assignment_id:
        return failure_response("Submission does not match this assignment")
