...
def get_all_subtasks():
    ...
    Get all subtasks
    """
    return success_response({"subtasks": DB.get_all_substask()})

@app.route("/tasks/<int:task_id>/subtasks/", methods=["POST"])
def create_subtask(task_id):
    """
    Create subtask for task_id
    """
    body = json.loads(request.data)
    description = body.get("description")
    ...
    if subtask is None:
        return failure_response("Did not sucessfully create subtask")
    return success_response(subtask, 201)

@app.route("/tasks/<int:task_id>/subtasks/")
def get_all_subtasks_for_task(task_id):
    """
    Get all subtask for task with task_id
    """
    task = DB.get_task_by_id(task_id)
    if task is None:
        return failure_response("Task not found")
    subtasks = DB.get_subtask_by_task(task_id)
