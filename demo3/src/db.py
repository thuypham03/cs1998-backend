...
#-- SUBTASKS --------------------------------------------------------   
    ...
    def insert_subtask(self, description, done, task_id):
        ...
        cursor = self.conn.execute(
            """INSERT INTO subtasks (description, done, task_id)
            VALUES (?, ?, ?);
    ...
    def get_subtask_by_task(self, task_id):
        """
        Use SQL, get all subtasks for task
        """
        cursor = self.conn.execute("SELECT * FROM subtasks WHERE task_id = ?", (task_id,))
        subtasks = []
        for row in cursor:
            subtasks.append(
                {
