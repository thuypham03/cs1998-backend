...
def create_user_extra():
    ...
    if password == -1:
        return failure_response("Missing password", 401)

    user_id = DB.insert_users_table(name, username, balance, password)
...
def get_user_extra(user_id):
    ...
    user_password = DB.get_password_by_id(user_id)
    if user_password is not None and user_password != password:
...
def send_money_extra():
    ...
    sender_password = DB.get_password_by_id(sender_id)
    if sender_password is not None and sender_password != password:
        return failure_response("Wrong password", 401)
